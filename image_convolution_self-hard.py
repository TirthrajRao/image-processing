...
# image = cv2.imread('paint1-soft.png') # reads the image
print(len(image))
...
def convolutionImage(topLeft, topMiddle, topRight, middleLeft, middle, middleRight, bottomLeft, bottomMiddle, bottomRight):
	...
	first = (int(topLeft[0]) * 0) + (int(topMiddle[0]) * -1) + (int(topRight[0]) * 0) + (int(middleLeft[0]) * -1) + (int(middle[0]) * 5) + (int(middleRight[0]) * -1) + (int(bottomLeft[0]) * 0) + (int(bottomMiddle[0]) * -1 ) + (int(bottomRight[0]) * 0)
	# print("first ==> ", first)
	first = round(first / 1)
	# print("first / 8==> ", first)
	arr.append(first)
	# arr.append((topLeft[0] + topMiddle[0] + topRight[0] + middleLeft[0] + middle[0] + middleRight[0] + bottomLeft[0] + bottomMiddle[0] + bottomRight[0])/ 5)
	second = (int(topLeft[1])  * 0 ) + (int(topMiddle[1])  * -1 ) + (int(topRight[1])  * 0  ) + (int(middleLeft[1])  * -1 ) + (int(middle[1])  * 5 ) + (int(middleRight[1])  * -1 ) + (int(bottomLeft[1])  * 0 ) + (int(bottomMiddle[1])  * -1 ) + (int(bottomRight[1])  * 0 )
	# print("second ==> ", second)
	second = round(second / 1)
	# print("second / 8==> ", second)

	arr.append(second)
	third = (int(topLeft[2])  * 0) + (int(topMiddle[2])  * -1 ) + (int(topRight[2])  * 0 ) + (int(middleLeft[2])  * -1 ) + (int(middle[2])  * 5 ) + (int(middleRight[2])  * -1 ) + (int(bottomLeft[2])  * 0 ) + (int(bottomMiddle[2])  * -1 ) + (int(bottomRight[2])  * 0 )
	# print("third ==> ", third)
	third = round(third / 1)
	# print("third / 8==> ", third )
	arr.append(third)
